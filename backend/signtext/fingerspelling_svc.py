...
PROTOTYPES: Dict[str, np.ndarray] = {
    "A": np.asarray([0.9791, 0.0026, 0.0026, 0.0026, 0.0026, 0.7033, 0.8994, 1.1454, 1.3797, -0.2252, -0.2620, -0.2770, -0.2251, -0.3326, -0.3812, 0.8210, 0.0004, 0.0925, 0.2696, -0.6366, 0.5502, 0.2827, -0.2735, -0.2119], dtype=np.float32),
    "B": np.asarray([0.7730, 1.0000, 1.0000, 1.0000, 1.0000, 1.6105, 1.8283, 1.6661, 1.2984, 1.3834, 1.6166, 1.4968, 1.1726, -0.0314, 0.0446, 0.9655, 0.9315, 0.1098, 0.4083, -0.0565, 0.6107, 0.3557, -0.0934, -0.0796], dtype=np.float32),
    "C": np.asarray([1.0000, 0.9315, 0.8904, 0.9808, 1.0000, 1.9328, 1.9521, 1.9818, 1.9844, 1.7668, 1.7898, 1.7970, 1.7464, -0.1924, 1.1362, 0.9162, 0.5618, 0.8197, 0.3078, 0.1561, 0.5239, 0.4310, -0.0440, -0.0163], dtype=np.float32),
    "D": np.asarray([0.5205, 1.0000, 0.0091, 0.0500, 0.4091, 1.9985, 0.5245, 0.6150, 0.8244, 1.9644, 0.4679, 0.4400, 0.6988, -0.6101, 0.5222, 1.0000, 0.9844, 0.2771, 1.9955, 0.3557, 1.9989, 0.3057, 0.0761, 0.2377], dtype=np.float32),
    "E": np.asarray([0.1575, 0.0334, 0.0215, 0.0239, 0.0406, 0.7527, 0.7858, 0.8464, 1.0140, 0.3289, 0.3066, 0.2554, 0.2778, -0.1342, 0.3470, 0.9656, 0.2874, 0.1295, 0.3488, -0.2780, 0.6681, 0.3336, -0.2225, -0.1767], dtype=np.float32),
    "F": np.asarray([0.2822, 0.1556, 1.0000, 0.9933, 0.9978, 0.2670, 1.7587, 1.7420, 1.5278, 0.2898, 1.6563, 1.5739, 1.2337, -0.1992, -0.0690, 0.9958, 0.1753, 0.1592, 1.5936, -0.5169, 1.6128, 0.3481, -0.1252, -0.1375], dtype=np.float32),
    "G": np.asarray([1.0000, 0.9978, 0.0000, 0.0000, 0.0022, 1.8614, 1.4524, 1.6896, 1.8924, 1.1142, -0.0358, -0.0077, -0.0397, 0.0767, -1.3299, 0.9997, 0.0237, 0.9998, 1.3189, -0.6727, 1.4888, 0.2765, 0.0114, -0.7216], dtype=np.float32),
    "H": np.asarray([1.0000, 1.0000, 1.0000, 0.0448, 0.0045, 0.9870, 1.0535, 0.4397, 0.6213, 1.1938, 1.5157, 0.2666, 0.1752, -0.1585, 0.2098, 1.0000, 0.0180, 1.0000, 0.3301, 0.1510, 1.3725, 1.4467, 0.0280, 0.0664], dtype=np.float32),
    "I": np.asarray([0.7500, 0.0000, 0.0000, 0.0000, 0.9977, 0.6529, 0.7666, 0.9050, 1.4652, -0.3036, -0.4703, -0.5589, 1.1982, -0.1463, -0.1399, 0.9993, 0.0004, 0.1301, 0.3019, 0.0265, 0.5267, 0.2511, -0.1139, -0.0790], dtype=np.float32),
    "J": np.asarray([0.9955, 0.1306, 0.1149, 0.2252, 0.9482, 0.7959, 0.9988, 1.2284, 1.5965, 0.1379, 0.2200, 0.3322, 1.0062, -0.4141, -0.9144, 0.8799, 0.0013, 0.7010, 0.2882, -0.1881, 0.5275, 0.2499, 0.1102, -0.0505], dtype=np.float32),
    "K": np.asarray([1.0000, 1.0000, 1.0000, 0.0971, 0.1625, 0.9561, 1.1271, 1.2220, 1.3891, 1.4635, 1.3407, 0.2107, 0.3267, -0.4282, -0.3679, 1.0000, 0.3797, 0.9126, 1.3560, 0.0272, 1.8284, 1.3819, 0.0222, 0.0405], dtype=np.float32),
    "L": np.asarray([1.0000, 0.9954, 0.0000, 0.0000, 0.0000, 1.9257, 1.4724, 1.7034, 1.9152, 1.2519, -0.4656, -0.4800, -0.3327, -0.3829, 0.3926, 0.9996, 0.9897, 0.1734, 1.7854, -0.4628, 1.8802, 0.2464, -0.1682, -0.2261], dtype=np.float32),
    "M": np.asarray([0.9505, 0.0112, 0.0112, 0.0112, 0.0080, 0.7590, 0.6314, 0.4999, 0.7860, -0.3174, -0.3475, -0.2630, -0.4989, 0.0891, -0.1679, 0.6801, 0.0101, 0.1605, 0.2467, -0.4757, 0.5001, 0.2649, -0.2391, -0.1871], dtype=np.float32),
    "N": np.asarray([0.8933, 0.0000, 0.0059, 0.0000, 0.0000, 0.6850, 0.5589, 0.8749, 0.8680, -0.2903, -0.2680, -0.6050, -0.4890, -0.0109, -0.3047, 0.7212, 0.0017, 0.1372, 0.2221, -0.5362, 0.5321, 0.4371, -0.2903, -0.1650], dtype=np.float32),
    "O": np.asarray([0.7524, 0.4222, 0.2540, 0.3206, 0.4889, 1.0013, 0.9369, 1.0402, 1.2394, 1.3651, 1.3115, 1.3058, 1.3690, -0.2637, -0.2215, 0.6383, 0.1603, 0.8454, 0.4133, 0.3699, 0.5517, 0.4259, -0.0018, 0.0073], dtype=np.float32),
    "P": np.asarray([0.9975, 1.0000, 0.9779, 0.0883, 0.1546, 0.7328, 0.9322, 0.9652, 1.1438, 1.2828, 1.2788, 0.4024, 0.3907, -0.2162, 0.8611, 1.0000, 0.0000, 0.5227, 1.0688, -0.1367, 1.5849, 1.1280, -0.4484, -0.7793], dtype=np.float32),
    "Q": np.asarray([0.9904, 0.9631, 0.1234, 0.0401, 0.0625, 1.1879, 1.3020, 1.4188, 1.5194, 1.6027, 0.2617, 0.0365, 0.0645, -0.4712, 1.6951, 0.9691, 0.0010, 0.3348, 1.4091, -1.4396, 1.5695, 0.3827, -0.2816, -0.1119], dtype=np.float32),
    "R": np.asarray([0.5451, 0.9954, 0.9909, 0.0000, 0.0000, 1.4566, 1.7085, 0.3703, 0.5863, 1.2663, 1.5393, -0.3393, -0.3562, 0.0098, -0.0759, 0.9996, 0.9469, 0.1575, 0.3323, -0.0422, 1.7900, 1.9231, -0.0209, 0.0999], dtype=np.float32),
    "S": np.asarray([0.9530, 0.0063, 0.0031, 0.0000, 0.0000, 0.5678, 0.3958, 0.4776, 0.6062, -0.1233, -0.1503, -0.2366, -0.2256, 0.0212, -0.2735, 0.7891, 0.0146, 0.0998, 0.2891, -0.5949, 0.5495, 0.2804, -0.2888, -0.2162], dtype=np.float32),
    "T": np.asarray([0.7585, 0.4921, 0.0000, 0.0000, 0.0000, 0.5183, 1.1419, 1.4578, 1.7034, 0.8868, -0.3030, -0.3166, -0.1987, -0.1954, -0.8180, 0.9982, 0.3864, 0.8175, 1.4072, -0.2711, 1.6976, 0.3711, 0.1440, -0.1311], dtype=np.float32),
    "U": np.asarray([0.4911, 1.0000, 1.0000, 0.0000, 0.0000, 1.5741, 1.7186, 0.4019, 0.5922, 1.3679, 1.5852, -0.3545, -0.3709, -0.0032, -0.1156, 1.0000, 0.9820, 0.0924, 0.3160, 0.1728, 1.8858, 1.9734, -0.0850, -0.0677], dtype=np.float32),
    "V": np.asarray([0.5556, 1.0000, 0.9952, 0.0000, 0.0072, 1.7657, 1.7709, 0.3470, 0.4867, 1.3773, 1.5125, -0.3877, -0.3502, -0.0085, 0.0229, 1.0000, 0.9541, 0.2927, 1.0237, 0.1394, 1.9518, 1.9673, -0.2673, -0.4129], dtype=np.float32),
    "W": np.asarray([0.8702, 1.0000, 1.0000, 0.9954, 0.0091, 1.9197, 1.9252, 1.6699, 0.2929, 1.3908, 1.5488, 1.4090, -0.2624, 0.0253, 0.1538, 0.9998, 0.9271, 0.3352, 0.8626, 0.0130, 1.4782, 0.7608, -0.0918, -0.1366], dtype=np.float32),
    "X": np.asarray([0.7470, 0.9642, 0.0095, 0.0024, 0.0072, 1.2914, 0.4989, 0.7440, 1.0166, 1.1007, -0.0014, 0.0002, 0.0389, -0.1712, -0.1167, 0.9997, 0.6983, 0.6610, 1.4750, -0.4770, 1.7346, 0.3344, 0.1633, -0.0551], dtype=np.float32),
    "Y": np.asarray([0.9977, 0.0023, 0.0000, 0.0000, 0.9910, 1.3346, 1.5913, 1.7472, 1.9994, -0.4870, -0.6499, -0.6197, 1.1119, -0.2626, -0.4744, 0.9980, 0.0020, 0.1670, 0.2923, 0.0314, 0.4612, 0.2033, -0.0669, -0.0373], dtype=np.float32),
    "Z": np.asarray([0.4199, 0.9223, 0.0194, 0.0291, 0.0461, 0.9403, 0.4165, 0.6289, 0.8034, 0.8676, -0.2600, -0.3683, -0.2896, -0.2510, -0.2047, 0.9890, 0.7403, 0.2177, 1.2375, -0.1343, 1.4962, 0.3150, -0.5624, -0.0977], dtype=np.float32),
}
